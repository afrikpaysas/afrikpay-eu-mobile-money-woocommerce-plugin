<?php
/*
 * Plugin Name: Express Union Mobile Money Payments for WooCommerce
 ...
 * Version: 1.0
 * Text Domain: expressunion-payments-for-woocommerce
 ...
 */
define( 'WC_EU_PAYMENTS_VERSION', '1.0' );
define( 'WC_EU_PAYMENTS_MIN_PHP_VER', '5.4.0' );
define( 'WC_EU_PAYMENTS_MIN_WC_VER', '3.0.0' );
define( 'WC_EU_PAYMENTS_MAIN_FILE', __FILE__ );
define( 'WC_EU_PAYMENTS_PLUGIN_PATH', untrailingslashit( plugin_dir_path( __FILE__ ) ) );

if ( ! class_exists( 'WC_EU' ) ) {
	...
	class WC_EU {
		...
		public function order_management_check() {
			if ( ! class_exists( 'WC_EU_Order_Management' ) ) {
				$current_screen = get_current_screen();
				if ( 'shop_order' === $current_screen->id || 'plugins' === $current_screen->id || 'woocommerce_page_wc-settings' === $current_screen->id ) {
					?>
					<div class="notice notice-warning">
						<p><?php _e( 'Afrikpay Order Management is not active. Please activate it so you can capture, cancel, update and refund Afrikpay orders.', 'woocommerce-expressunion-payments' ); ?></p>
					</div>
					<?php
				}
			}
		...
		public function plugin_action_links( $links ) {
			...
			$plugin_links = array(
				'<a href="' . $setting_link . '">' . __( 'Settings', 'expressunion-payments-for-woocommerce' ) . '</a>',
				'<a href="http://afrikpay.cederconsulting.com/">' . __( 'Support', 'expressunion-payments-for-woocommerce' ) . '</a>',
			);
		...
		public function get_setting_link() {
			...
			$section_slug = $use_id_as_section ? 'expressunion' : strtolower( 'WC_Gateway_EU' );
		...
		public function init_gateways() {
			...
			include_once( WC_EU_PAYMENTS_PLUGIN_PATH . '/includes/class-wc-gateway-expressunion-payments.php' );
			//include_once( WC_EU_PAYMENTS_PLUGIN_PATH . '/includes/class-wc-afrikpay-payments-order-lines.php' );

			load_plugin_textdomain( 'expressunion-payments-for-woocommerce', false, plugin_basename( dirname( __FILE__ ) ) . '/languages' );
		...
		public static function log( $message ) {
			...
			self::$log->add( 'expressunion-payments-for-woocommerce', $message );
		}
		...
		public function maybe_filter_billing_phone( $phone_value ) {
			...
			if ( 'expressunion_payments' === $_POST['payment_method'] ) { // Input var okay.
